...
public class AuthorizationServiceImpl implements AuthorizationService {
	...
	public void init() throws Exception {
		InputStreamReader reader = new InputStreamReader(
				new ClassPathResource(config.getDriveSecretKeys()).getInputStream());
		...
		flow = new GoogleAuthorizationCodeFlow.Builder(ApplicationConstant.HTTP_TRANSPORT,
				ApplicationConstant.JSON_FACTORY, clientSecrets, ApplicationConstant.SCOPES)
	...
	public void exchangeCodeForTokens(String code) throws Exception {
		...
		GoogleTokenResponse tokenResponse = flow.newTokenRequest(code).setRedirectUri(config.getCALLBACK_URI())
				.execute();
